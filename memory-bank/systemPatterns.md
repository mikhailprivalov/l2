...
- **DOU document manager** (`/ui/document-manager`): a `Documents` row has `Issledovaniya.document`; field values are `ParaclinicResult` on that iss (same payload as `directions/paraclinic_result`, without direction-only side effects). Save stays on `document-manager/documents/save` because DOU users have group **ДОУ: просмотр документов**, not the doctor groups for `paraclinic_result`. Confirm writes `Issledovaniya.time_confirmation` and mirrors `Documents.time_confirm` / `who_confirm`. Confirm reset is limited to Django group **Сброс документов** (or superuser); details returns `canReset`. Actions write `slog.Log` with types 260000–260005 (`key` = document pk). History UI `/ui/document-history` (group **История документа**) matches `/ui/direction/history` (search by number, grouped event cards) and reads only those types via `document-manager/documents/history` — not `/ui/logs`. **Найти** looks up `Documents.pk` via `documents/find` (`Documents.find_by_id`); changing group reloads types only, not the document list. Field type 45 **Адресат** stores JSON `[{id, fio}]`; save syncs `DocumentReview.doctor_review` for the document. Addressee sets are `AddresseeGroup` / `AddresseeGroupMember`. Surrogate group **Все** is only for Django group **Все адресаты** or superuser (constructor may still search all employees). Constructor sets have empty `who_create`; personal sets from the document modal belong to the current doctor and are listed only for that user. Opening a confirmed document (`documents/details`) stamps `time_review` on that user's pending `DocumentReview` row. **Последние** is `DocumentRecent` (per doctor, unique document, max 200, page 50). List title is topic then type (`" ".join(topic, type_doc)`). Topic is the value of the field whose `cda_option` is `CDA_TOPIC_ID_FOR_DOCUMENT_MANAGER` (`laboratory.settings` / `local_settings`, default `-1` disables). Lookup uses the document schema snapshot (`cdaOption` on fields), then live `ParaclinicInputField` + `ParaclinicResult` / `body_values`. `documents/recent` re-reads the current document topic; save and open also refresh stored `DocumentRecent.topic`. Schema dump now stores `cdaOption` on each field.
