...
- `hospitals/` - hospitals
- `users/` - users and doctor profiles. `/ui/profiles` saves a signature stamp image (`signature_stamp_pdf` in `docprofile_stamp_pdf`) plus `width_stamp_jpg` / `height_stamp_jpg` in millimetres. `/ui/profiles` **Сотрудник из графика** links `DoctorProfile` to construct `EmployeePosition` via `DoctorProfileEmployeePosition`. Options are a two-level tree (`get_employee_positions_tree`): department nodes (`dept-{pk}`, not selectable) and leaf nodes with `EmployeePosition.pk`. Only active position + employee + department rows for the selected hospital. Search is client-side (`search-nested`) by department name or FIO; if one employee has several positions in the same department, the leaf label is `FIO — position`.
- `employees/` - staff (`Employee`, `EmployeePosition`, catalog `Position`). People modal: optional `date_birth` and `snils`. Position modal: `is_active` (**Активна**), mutually exclusive `is_mentee` / `is_mentor`. Protocol field types 46 **Наставляемый** and 47 **Наставник** pick those positions (`employees/mentees`, `employees/mentors`); mentee tree is department → FIO + `date_employment`; mentor list is FIO in the selected mentee's department. Not dismissed: `date_dismissal` empty or in the future.
...
- **Hospital research synonym:** `TitleResearchHospital` stores a per-hospital service title. Constructor `/ui/construct/price` shows an editable column after the research name (save with the price row). Excel `100.01` reads/writes column `Синоним`. Both save only for **Заказчик** prices that have a hospital; empty values are skipped. On `/ui/request-creation`, if the current user's hospital has a synonym, it replaces the service title in the assignment picker, the request list, and result print (`result_print` / schema docx). `Hospitals.only_services_with_hospital_synonym` (default off) limits that picker to services that have a synonym and rejects `requests/create` without one. `/ui/requests-fill` cards and the protocol title (`directions/paraclinic_form` when `is_request`) use the synonym of the request hospital (`Napravleniya.hospital`), including when the describing doctor belongs to another organization.
...
- **Gardening accounting (Все):** In Учёт mode with «Все» selected, the header shows year payment types. If the selected type is electricity (`is_use_kilowatt` or title contains «электроэнергия»), a January–December month strip appears below the types, plus **Долги** after December. Clicking a month opens `GardeningElectricityMonthList`: one row per active meter across plots (plot, meter, previous/current readings with date headers, consumption, tariff, charge, written-off, plot totals: consumption, written-off, debt, remainder, receipt). Plot-total columns (`* общ`) show values only on the first visible row of each plot after filters/sort. Column sort; filters for debt and missing current reading. Inline reading edit uses the same create/update reading APIs as the plot electricity table. API: `gardening/get-electricity-month-rows`. **Долги** opens `GardeningDebtsList`: plots whose **cumulative** unpaid electricity debt through the as-of month is > 0 (today’s month of the selected year, or December if the year is past). Charge / written-off / debt are sums of monthly values from the start of the calculation through that month; remainder is the as-of snapshot. July’s unpaid charge still appears in September if it was not covered. API: `gardening/get-payment-type-debts`. Other payment types (except `not_control`) open `GardeningAccountingSummary` in table mode: one row per plot with the same fields and formulas as plot **Взносы** (`_contribution_row`): tariff, coefficient, charge, written-off, debt, remainder. Missing tariff/area/charge is red `0.00`. Column sort and a debt filter. «Итого» still lists types with bank receipt totals. API: `gardening/get-accounting-summary`. The **Участок** column in both all-plots tables is a link `/ui/gardening?id=<plot>&year=<year>` (`target=_blank`); the page reads these query params on load and opens that plot card in Учёт. **PDF** and **Excel** on each Все view print the current table (`forms` type `115.02` / `forms.xlsx.forms115.form_02`): same columns, filters, and sort as on screen. Helper `_all_plots_export` builds the payload. In **Настройка**, **Бэкап SQL** (next to База) downloads a gzipped full `pg_dump` of the PostgreSQL database (`gardening/backup-sql`).
...
- Extra notifications: `EXTRA_MASTER_RESEARCH_PK` is a list of master research pks (int still accepted); SQL filters with `research_id in %(master_research)s` after converting to a tuple. `EXTRA_SLAVE_RESEARCH_PK` stays a single slave research pk. `/ui/extra-notification` filters those masters with checkboxes (short titles) above the table; the **Услуга** column shows full titles. Search returns `researches` from the setting only.
...
- **ECP lab directions import** (`directions/get-from-ecp`): `get_directions_from_ecp` returns `{"result": [{direction, EvnLabRequest_id, Person_id, UslugaComplex_id, ...}]}`. Request `endDate` is today; `starDate` is today plus `LIPID_RESEARCHES_DAYS_AGO_FIND` days (`0` keeps both dates equal, `-1` is yesterday). Rows that share `direction` become one card. `UslugaComplex_id` maps to L2 research pks via `LIPID_RESEARCHES`. The modal shows `direction_id` as the ECP direction number and `evnLabRequestId` as the request. An already registered card shows `Направление уже зарегистрировано в системе - {Napravleniya.pk}` (`localDirectionId`). Direction PDF blanks (`/ui/directions/preview`) print `№-e {pk}` when `is_external` and `ecp_direction_number` are set (`Napravleniya.external_number_suffix`). The 80 mm blank has no № sign and prints `Направление -e {pk}`. Each card creates a local direction through `directions/create-from-ecp` (`EvnDirectionId` → `ecp_direction_number` / `rmis_number`, `EvnLabRequest_id` → `request_code`, `is_external=True`).
...
- **DOU constructor** (`/ui/construct/dou`): groups, types, **Виды дел** (`TypeCases`: title, code, `default_type_document`), structure, and **Шаблоны документов** (`is_layout_template` researches via `ParaclinicResearchEditor`, department `-17`). Layout templates are no longer created under `/ui/construct/descriptive` (**Шаблоны**). **Формы** stay on descriptive. Type editor **Создатели** assigns who may create that type: addressee-style picker (constructor sets or specific employees; “add set” snapshots members). Stored as `TypeDocumentCreator` (`type_document` + `doctor`). Empty creators = no restriction. Case types APIs: `document-manager/cases/list` and `document-manager/cases/update`.
- **DOU document manager** (`/ui/document-manager`): `DOCUMENT_MANAGER_FOR_ALL` in `laboratory.settings` / `local_settings` (default `False`) is an on/off switch: when `True`, the menu button, page, and `ДОУ: просмотр документов` APIs ignore groups and are open to every authenticated user (`modules.document_manager_for_all` on the frontend). When `False`, access stays on group **ДОУ: просмотр документов**. Constructor, history, hide, and confirm-reset stay on their groups. A `Documents` row has `Issledovaniya.document`; field values are `ParaclinicResult` on that iss (same payload as `directions/paraclinic_result`, without direction-only side effects). Save stays on `document-manager/documents/save` because DOU users have group **ДОУ: просмотр документов**, not the doctor groups for `paraclinic_result`. Confirm writes `Issledovaniya.time_confirmation` and mirrors `Documents.time_confirm` / `who_confirm`. Confirm reset is limited to Django group **Сброс документов** (or superuser); details returns `canReset`. Hide is allowed for the document author until confirm, plus superuser and Django group **Скрытие документа**; details returns `canHide`. Hidden documents are listed/opened only for that group and superuser; the **Скрытые** checkbox is shown only then. Actions write `slog.Log` with types 260000–260005 (`key` = document pk). History UI `/ui/document-history` (group **История документа**) matches `/ui/direction/history` (search by number, grouped event cards) and reads only those types via `document-manager/documents/history` — not `/ui/logs`. **Найти** looks up `Documents.pk` via `documents/find` (`Documents.find_by_id`); changing group reloads types only, not the document list. Field type 45 **Адресат** stores JSON `[{id, fio}]`; save syncs `DocumentReview.doctor_review` for the document. Addressee sets are `AddresseeGroup` / `AddresseeGroupMember`. Surrogate group **Все** is only for Django group **Все адресаты** or superuser (constructor may still search all employees). Constructor sets have empty `who_create`; personal sets from the document modal belong to the current doctor and are listed only for that user. Opening a confirmed document (`documents/details`) stamps `time_review` on that user's pending `DocumentReview` row without reloading `Issledovaniya`. Repeated opens update `DocumentRecent` without trimming the 200-row cap. **Новые** (`pending_count` / `pending_document_ids`) excludes `Documents.is_hidden`. **Последние** is `DocumentRecent` (per doctor, unique document, max 200, page 50). List title is topic then type (`" ".join(topic, type_doc)`). Topic is the value of the field whose `cda_option` is `CDA_TOPIC_ID_FOR_DOCUMENT_MANAGER` (`laboratory.settings` / `local_settings`, default `-1` disables). Lookup uses the document schema snapshot (`cdaOption` on fields), then live `ParaclinicInputField` + `ParaclinicResult` / `body_values`. `documents/recent` re-reads the current document topic; save and open also refresh stored `DocumentRecent.topic`. Schema dump now stores `cdaOption` on each field. Type list on this page calls `types/list` with `availableOnly`: empty `TypeDocumentCreator` set is visible to everyone; otherwise only listed doctors. `documents/create` uses `TypeDocuments.can_create`. Constructor `types/list` is unfiltered.
...
- **DOU document manager** (`/ui/document-manager`): `DOCUMENT_MANAGER_FOR_ALL` in `laboratory.settings` / `local_settings` (default `False`) is an on/off switch: when `True`, the menu button, page, and `ДОУ: просмотр документов` APIs ignore groups and are open to every authenticated user (`modules.document_manager_for_all` on the frontend). When `False`, access stays on group **ДОУ: просмотр документов**. Constructor, history, hide, and confirm-reset stay on their groups. A `Documents` row has `Issledovaniya.document`; field values are `ParaclinicResult` on that iss (same payload as `directions/paraclinic_result`, without direction-only side effects). Save stays on `document-manager/documents/save` because DOU users have group **ДОУ: просмотр документов**, not the doctor groups for `paraclinic_result`. Confirm writes `Issledovaniya.time_confirmation` and mirrors `Documents.time_confirm` / `who_confirm`. Confirm reset is limited to Django group **Сброс документов** (or superuser); details returns `canReset`. Hide is allowed for the document author until confirm, plus superuser and Django group **Скрытие документа**; details returns `canHide`. Hidden documents are listed/opened only for that group and superuser; the **Скрытые** checkbox is shown only then. Actions write `slog.Log` with types 260000–260005 (`key` = document pk). History UI `/ui/document-history` (group **История документа**) matches `/ui/direction/history` (search by number, grouped event cards) and reads only those types via `document-manager/documents/history` — not `/ui/logs`. **Найти** looks up `Documents.pk` via `documents/find` (`Documents.find_by_id`); changing group reloads types only, not the document list. Field type 45 **Адресат** stores JSON `[{id, fio}]`; save syncs `DocumentReview.doctor_review` for the document. Addressee sets are `AddresseeGroup` / `AddresseeGroupMember`. Surrogate group **Все** is only for Django group **Все адресаты** or superuser (constructor may still search all employees). Constructor sets have empty `who_create`; personal sets from the document modal belong to the current doctor and are listed only for that user. Opening a confirmed document (`documents/details`) stamps `time_review` on that user's pending `DocumentReview` row without reloading `Issledovaniya`. `documents/details` prefetches `Issledovaniya`, loads `ParaclinicResult` without joining `ParaclinicInputField`/`input_templates`, reuses the latest type schema when `Documents.schema` is missing, and checks hide/reset groups in one query. Repeated opens update `DocumentRecent` without trimming the 200-row cap. **Новые** (`pending_count` / `pending_document_ids`) excludes `Documents.is_hidden`; list titles are `{id} {title}` (`Documents.title_with_id`, no `№`). **Последние** is `DocumentRecent` (per doctor, unique document, max 200, page 50; `get_page` keeps only `is_hidden=False` and `time_confirm` set; same `{id} {title}`). List title is topic then type (`" ".join(topic, type_doc)`). Topic is the value of the field whose `cda_option` is `CDA_TOPIC_ID_FOR_DOCUMENT_MANAGER` (`laboratory.settings` / `local_settings`, default `-1` disables). Lookup uses the document schema snapshot (`cdaOption` on fields), then live `ParaclinicInputField` + `ParaclinicResult` / `body_values`. `documents/recent` re-reads the current document topic; save and open also refresh stored `DocumentRecent.topic`. Schema dump now stores `cdaOption` on each field.
...
- **ISS attached files:** `appconf.Setting` `iss_keep_only_latest_file` (boolean, default off). When on, `IssledovaniyaFiles.keep_only_latest` after `directions/add-file` deletes older files for that iss and keeps the newest. Frontend (`FileAddModal`) still shows upload so a new file can replace the old one. `result_print` deduplicates `Issledovaniya.link_file` and `IssledovaniyaFiles` PDF paths so the same attachment is not printed twice.
