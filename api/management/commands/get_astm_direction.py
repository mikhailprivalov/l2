...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(json.dumps(get_astm(Napravleniya.objects.filter(pk__in=options['directions'].split(",")), Analyzer.objects.get(pk=options['analyzer_id']), True, out=self.stdout)))
