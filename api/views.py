...
def construct_menu_data(request):
    ...
    pages = [
        ...
        {"url": "/ui/construct/tube-barcode", "title": "Штрих-коды ёмкостей", "access": ["Конструктор: Ёмкости для биоматериала"], "module": None},
        ...
        {"url": "/ui/construct/dou", "title": "ДОУ-конструктор", "access": ["Конструктор: ДОУ"], "module": None},
    ]
...
def get_types_magazine(request):
    data = [{"id": k, "label": v.get("label"), "showPeriodDate": v.get("show_date")} for k, v in MAGAZINE_REPORT.items()]
    return JsonResponse({"data": data})
