...
def forbidden_edit_dir(num_dir):
    """
    Проверяет подтверждена ли выписка, или переводной эпикриз. И возвращает True|False - для права редактирвоания протколов
    """
    ...
    if not hosp_extract:
        #получить для текущего hosp_dir (num_dir на входе) эпикриз с title - перевод.....
        epicrisis_data = hosp_get_data_direction(num_dir, site_type=6, type_service='None', level=2)
        if epicrisis_data:
