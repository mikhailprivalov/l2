...
def get_plans_by_params_sql(d_s, d_e, doc_operate_id, doc_anesthetist_id, department):
    ...
    with connection.cursor() as cursor:
        ...
        t_plans AS 
            ...
            type_operation, doc_operate_id, doc_anesthetist_id, canceled, date, duration
        ...
        SELECT pk_plan, patient_card_id, direction, date_char, type_operation, doc_operate_id, doc_anesthetist_id, canceled,
               ind_family, ind_name, ind_twoname, birthday, date, duration FROM t_plans
        LEFT JOIN t_patient ON t_plans.patient_card_id = t_patient.card_id 
        LEFT JOIN users_doctorprofile ON users_doctorprofile.id=doc_operate_id
        ORDER BY date, users_doctorprofile.podrazdeleniye_id
...
def get_plans_by_pk(pks_plan):
    ...
    with connection.cursor() as cursor:
        ...
        doc_anesthetist_id, t_users_anesthetist.fio, canceled, ind_family, ind_name, ind_twoname, birthday, date, t_users_doc.podr_title, duration  FROM t_plans
        LEFT JOIN t_patient ON t_plans.patient_card_id = t_patient.card_id
