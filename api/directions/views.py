...
def eds_documents(request):
    ...
    if not iss_obj.doc_confirmation.podrazdeleniye.n3_id:
        return JsonResponse({"documents": [], "edsTitle": "", "executors": "", "error": True, "message": "UUID подразделения не заполнен"})
...
def eds_to_sign(request):
    ...
    if number:
        ...
        if mode == 'mo':
            ...
            if not request.user.doctorprofile.podrazdeleniye.n3_id:
                return JsonResponse({"rows": rows, "page": page, "pages": 0, "total": 0, "error": True, "message": "UUID подразделения не заполнен"})
