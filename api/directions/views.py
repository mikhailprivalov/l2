...
from laboratory.settings import (
    ...
    RMQ_RESEARCH_SEND, NOT_CONTROL_VISIT_RESEARCH_ID,
)
...
def directions_paraclinic_form(request):
    ...
    if dn.exists():
        ...
        if (
            ...
            and (d.visit_date is None or d.register_number is None) and d.research().pk not in NOT_CONTROL_VISIT_RESEARCH_ID
        ):
...
def directions_paraclinic_confirm_reset(request):
    ...
    if Issledovaniya.objects.filter(pk=pk).exists():
        ...
        if iss.napravleniye.need_resend_ecp and "Сброс для отправленных в РМИС" not in g:
