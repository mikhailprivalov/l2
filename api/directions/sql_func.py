...
def get_confirm_direction(d_s, d_e, is_lab=False, is_paraclinic=False, is_doc_refferal=False):
    with connection.cursor() as cursor:
        cursor.execute(
            """
        SELECT DISTINCT ON (napravleniye_id) napravleniye_id FROM public.directions_issledovaniya
        ...
        WHEN  %(is_lab)s = FALSE AND %(is_paraclinic)s = TRUE AND %(is_doc_refferal)s = TRUE  THEN 
          is_paraclinic = TRUE and is_doc_refferal = TRUE
        ...
        WHEN %(is_lab)s = TRUE AND %(is_paraclinic)s = FALSE AND %(is_doc_refferal)s = FALSE THEN
          is_paraclinic = FALSE and is_doc_refferal = FALSE and is_stom = FALSE and is_hospital = FALSE and is_microbiology = FALSE 
                and is_citology = FALSE and is_gistology = FALSE AND is_slave_hospital = FALSE
