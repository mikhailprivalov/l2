...
def form_02(request_data):
    ...
    title_page = [
        ...
        Spacer(1, 0.5 * mm),
        Paragraph(f"Предварительный диагноз (диагноз при поступлении): {primary_reception_data['diagnos_entered']}", style),
        Spacer(1, 0.5 * mm),
        Paragraph(f"Дата и время установления диагноза при поступлении: {primary_reception_data['date_preliminary_diagnosis']}г. время:	{primary_reception_data['time_preliminary_diagnosis']}", style),
        Spacer(1, 0.5 * mm),
        Paragraph(f"Основное заболевание код по МКБ: {primary_reception_data['final_diagnos_mkb']}", style),
        Spacer(1, 0.5 * mm),
        Paragraph(f"Осложнения основного заболевания код по МКБ: {primary_reception_data['other_diagnos_mkb']}", style),
        Spacer(1, 0.5 * mm),
        Paragraph(f"Внешняя причина при травмах, отравлениях   код по МКБ: {primary_reception_data['external_reason_mkb']}", style),
        Spacer(1, 0.5 * mm),
        Paragraph(f"Сопутствующие заболевания код по МКБ: {primary_reception_data['near_diagnos_mkb']}", style),

        Spacer(1, 0.5 * mm),
        Paragraph(f"Дополнительные сведения о заболевании {primary_reception_data['additional_data_ill']}", style),
