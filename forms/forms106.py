...
def form_02(request_data):
    ...
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("Оформлен листок нетрудоспособности: № от «		»  20		г. ", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("(дубликат листка нетрудоспособности №			от «	»			20	г.)", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("освобождение от работы с «	»	20	г. по «			»			20		г.", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("продление листка нетрудоспособности:", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("№	освобождение от работы с «	»	20	г. по «	»	20	г.", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("№	освобождение от работы с «	»	20	г. по «	»	20	г.", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("приступить к работе с «	»	20	г.", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("явка в другую медицинскую организацию (другое структурное подразделение медицинской организации) «	»	20  г. ", style))
    title_page.append(Spacer(1, 2 * mm))
    title_page.append(Paragraph("Оформлен листок нетрудоспособности по уходу за больным членом семьи (фамилия, имя, отчество (при наличии):", style))
    title_page.append(Paragraph("Выдано направление на медико-социальную экспертизу (МСЭ): «	»	20	г. ", style))
    title_page.append(Spacer(1, 2 * mm))
