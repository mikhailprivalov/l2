...
def form_14(request_data):
    ...
    objs.append(Paragraph('Подтверждаю, что я ознакомлен(на) с характером предстоящей представляемому операции. Мне разъяснены, и я понимаю особенности и ход предстоящего оперативного лечения.',
        style=style))
    objs.append(Paragraph('Мне разъяснено, и я осознаю, что во время операции могут возникнуть непредвиденные обстоятельства и осложнения. В таком случае я согласен(на) на то, что ход операции '
        'может быть изменен врачами по их усмотрению.', style=style))
    objs.append(Paragraph('Я предупрежден(на) о фактах риска и понимаю, что проведение операции сопряжено с риском потери крови, возможностью инфекционных осложнений, нарушений со стороны ' 
        'сердечно-сосудистой и других систем жизнедеятельности организма непреднамеренного причинения вреда здоровью и даже неблагоприятного исхода', style=style))
    objs.append(Paragraph('Я предупрежден(на), что в ряде случаев могут потребляться повторные операции, в т.ч. в связи с возможными послеоперационными осложнениями течения заболевания, '
        'и даю свое согласие на это', style=style))
    objs.append(Paragraph('Я поставил(ла) в известность врача обо всех проблемах, связанных со здоровьем, в том числе об аллергических проявлениях или индивидуальной непереносимости '
        ...
        'компонентов. Сообщил(а) правдивые сведения о наследственности, а также об употреблении алкоголя, наркотических и токсических средств.', style=style))
    ...
    objs.append(Paragraph('Я согласен(на) на запись хода операции на информационные носители и демонстрацию лицам с медицинским образованием исключительно медицинских, научных '
        'или обучающих целях с учетом сохранения врачебной тайны.', style=style))
    objs.append(Paragraph('Мне  была предоставлена возможность задать вопросы о степени риска и пользе хирургического вмешательства (манипуляции), и врач дал понятные мне исчерпывающие ответы.',
        style=style))
