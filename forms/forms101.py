...
def form_32(request_data):
    ...
    objs.append(
        Paragraph(
            '- гражданам иностранных государств, лицам без гражданства, за исключением лиц, застрахованных по обязательному медицинскому страхованию, и гражданам '
            'Российской Федерации, не проживающим постоянно на ее территории и не являющимся',
            styleSign,
        )
    )
    objs.append(
        Paragraph(
            '- при самостоятельном обращении за получением медицинских услуг, за исключением случаев и порядка, предусмотренных статьей 21 Федерального закона' ' № 323-ФЗ от 21.11.2011;',
            styleSign,
        )
    )
    objs.append(
        Paragraph(
            '6. Отказ потребителя (пациента) от предлагаемых платных медицинских услуг не может быть причиной уменьшения видов и объема оказываемой медицинской '
            ...
            'медицинской помощи и территориальной программы государственных гарантий бесплатного оказания гражданам медицинской помощи;',
            styleSign,
        )
    )
    objs.append(
        Paragraph(
            '7. Исполнитель уведомляет потребителя (пациента) о том, что несоблюдение указаний (рекомендаций) исполнителя (медицинского работника, предоставляющего '
            ...
            'невозможность ее завершения в срок или отрицательно сказаться на состоянии здоровья потребителя (пациента);',
            styleSign,
        )
    )
    objs.append(
        Paragraph(
            '8. Внимательно прочитав все вышеуказанное, я подтверждаю, что получил исчерпывающую информацию о лечении и его условиях в доступной для меня форме, и выбираю '
            'оказание платных медицинских услуг, о чем свидетельствует моя подпись под этим документом.',
            styleSign,
        )
    )
    current_date = strfdatetime(current_time(only_date=True), "%d.%m.%Y")
    objs.append(Paragraph('Потребитель(пациент)____________________<font face="Symbola" size=18>\u2713</font> /______________________ Дата:', styleSign))
    objs.append(Spacer(1, 7 * mm))
    objs.append(
        Paragraph(
    ...
    objs.append(
        Paragraph(
            f'В соответствии с требованиями ст. 9 ФЗ от 27.07.2006 г. "О персональных данных" №152-ФЗ, подтверждаю свое согласие на обработку в {hospital_name} '
    ...
    objs.append(
        Paragraph(
            'В процессе оказания Оператором мне медицинской помощи я предоставляю право медицинским работникам, передавать мои персональные данные, содержащие '
            ...
            'по ОМС (договором ДМС).',
            styleSign,
        )
    )
    objs.append(
        Paragraph(
            'Оператор имеет право во исполнение своих обязательств по работе ОМС (по договору ДМС) на обмен (прием и передачу) моими персональными данными с '
    ...
    objs.append(
        Paragraph(
            'Я оставляю за собой право отозвать свое согласие посредством составления соответствующего письменного документа, который может быть направлен мной в '
...
def form_33(request_data):
    ...
    objs.append(
        Paragraph(
            'в доступной для меня форме подробно информировал меня о состоянии здоровья моего ребёнка, о наличии у него заболевания, '
