...
def form_30(request_data):
    ...
    objs.append(
        Paragraph(
            '<b>Информированное согласие пациента на проведение обследования на ВИЧ-инфекцию</b>',
            style=styleHeader,
        )
    )
    objs.append(Spacer(1, space))
...
def form_32(request_data):
    """
    Два согласия одновременно профосмотр
    """
    ind_card = Card.objects.get(pk=request_data["card_pk"])
    person_data = ind_card.get_data_individual()
    if sys.platform == 'win32':
    ...
    objs.append(Spacer(1, 1 * mm))
    objs.append(
        Paragraph('до заключения договора о предоставлении платных медицинских услуг, в доступной для меня форме доведено следующее:', styleSign,
        )
    )

    objs.append(
        Paragraph(
            '1. Разъяснены мои права, предусмотренные: статьями 19, 21 Федерального закона № 323-ФЗ от 21.11.2011 «Об основах охраны здоровья граждан в Российской Федерации», '
            'Законом Российской Федерации № 2300-1 от 07.02.1992 «О защите прав',
            styleSign,
        )
    )

    objs.append(Paragraph('2. В соответствии с требованиями статей 20, 22 Федерального закона № 323-ФЗ от 21.11.2011, я информирован(а), что необходимым предварительным условием оказания '
                          'медицинской помощи является получение добровольного информированного согласия, а также имею право отказаться от получения медицинской помощи, и дал такое '
                          'согласие;', styleSign))
    objs.append(Paragraph('3. Мне разъяснено мое законное право на медицинскую помощь в гарантированном объеме, оказываемую без взимания платы в соответствии с программой государственных '
                          'гарантий бесплатного оказания гражданам медицинской помощи, территориальной программы, при соблюдении условий программ, а также положения данных '
                          'программ;', styleSign))
    objs.append(Paragraph('4. Мне разъяснено право на получение платных медицинских услуг, предоставляемых по моему желанию при оказании медицинской помощи, и платных немедицинских услуг '
                          '(бытовых, сервисных, транспортных и иных услуг), предоставляемых дополнительно при оказании медицинской помощи, за счет личных средств граждан, средств '
                          'работодателей и иных средств на основании договоров;', styleSign))
    objs.append(Paragraph('5. Платные медицинские услуги оказываются по моему желанию на иных условиях, чем предусмотрено программой, территориальными программами и (или) целевыми '
                          'программами в виде осуществления отдельных консультаций или медицинских вмешательств, в том числе в объеме, превышающем объем выполняемого стандарта '
                          'медицинской помощи:', styleSign))
    objs.append(Paragraph('- на иных условиях, чем предусмотрено программой государственных гарантий бесплатного оказания гражданам медицинской помощи, территориальными программами '
                          'государственных гарантий бесплатного оказания гражданам медицинской помощи и (или) целевыми программами;', styleSign))
    objs.append(Paragraph('- при оказании медицинских услуг анонимно, за исключением случаев, предусмотренных законодательством Российской Федерации;', styleSign))
    objs.append(Paragraph('- гражданам иностранных государств, лицам без гражданства, за исключением лиц, застрахованных по обязательному медицинскому страхованию, и гражданам '
                          'Российской Федерации, не проживающим постоянно на ее территории и не являющимся', styleSign))
    objs.append(Paragraph('- при самостоятельном обращении за получением медицинских услуг, за исключением случаев и порядка, предусмотренных статьей 21 Федерального закона'
                          ' № 323-ФЗ от 21.11.2011;', styleSign))
    objs.append(Paragraph('6. Отказ потребителя (пациента) от предлагаемых платных медицинских услуг не может быть причиной уменьшения видов и объема оказываемой медицинской '
                          'помощи, предоставляемых такому потребителю (пациенту) без взимания платы в рамках программы государственных гарантий бесплатного оказания гражданам '
                          'медицинской помощи и территориальной программы государственных гарантий бесплатного оказания гражданам медицинской помощи;', styleSign))
    objs.append(Paragraph('7. Исполнитель уведомляет потребителя (пациента) о том, что несоблюдение указаний (рекомендаций) исполнителя (медицинского работника, предоставляющего '
                          'платную медицинскую услугу), в том числе назначенного режима лечения, могут снизить качество предоставляемой платной медицинской услуги, повлечь за собой '
                          'невозможность ее завершения в срок или отрицательно сказаться на состоянии здоровья потребителя (пациента);', styleSign))
    objs.append(Paragraph('8. Внимательно прочитав все вышеуказанное, я подтверждаю, что получил исчерпывающую информацию о лечении и его условиях в доступной для меня форме, и выбираю '
                          'оказание платных медицинских услуг, о чем свидетельствует моя подпись под этим документом.', styleSign))
    ...
    styleLeft = deepcopy(style)
    styleLeft.alignment = TA_LEFT
    objs.append(Paragraph(f'В соответствии с требованиями ст. 9 ФЗ от 27.07.2006 г. "О персональных данных" №152-ФЗ, подтверждаю свое согласие на обработку в {hospital_name} '
                          '(далее – Оператор) моих персональных данных, включающих: фамилию, имя, отчество, пол, дату рождения, адрес проживания, контактный телефон, реквизиты полиса '
                          'ОМС (ДМС), страховой номер индивидуального лицевого счета в Пенсионном фонде России (СНИЛС), паспортные данные, данные о состоянии моего здоровья, заболеваниях, '
                          'случаях обращения за медицинской помощью, – в медико-профилактических целях, в целях установления медицинского диагноза и оказания медицинских услуг при '
                          'условии, что их обработка осуществляется лицом, профессионально занимающимся медицинской деятельностью и обязанным сохранять врачебную тайну.', styleSign))
    objs.append(Paragraph('В процессе оказания Оператором мне медицинской помощи я предоставляю право медицинским работникам, передавать мои персональные данные, содержащие '
                          'сведения, составляющие врачебную тайну, другим должностным лицам Оператора, в интересах моего обследования и лечения. Предоставляю Оператору право '
                          'осуществлять все действия (операции) с моими персональными данными, включая сбор, систематизацию, накопление, хранение, обновление, изменение, '
                          'использование, обезличивание, блокирование, уничтожение. Оператор вправе обрабатывать мои персональные данные посредством внесения их в электронную '
                          'базу данных, включения в списки (реестры) и отчетные формы, предусмотренные документами, регламентирующими предоставление отчетных данных (документов) '
                          'по ОМС (договором ДМС).', styleSign))
    objs.append(Paragraph('Оператор имеет право во исполнение своих обязательств по работе ОМС (по договору ДМС) на обмен (прием и передачу) моими персональными данными с '
                          'использованием машинных носителей или по каналам связи, с соблюдением мер, обеспечивающих их защиту от несанкционированного доступа, при условии, '
                          'что их прием и обработка будут осуществляется лицом, обязанным сохранять профессиональную тайну. Срок хранения моих персональных данных соответствует '
                          'сроку хранения первичных медицинских документов и составляет двадцать пять лет. Передача моих персональных данных иным лицам или иное их разглашение '
                          'может осуществляться только с моего письменного согласия.', styleSign))
    objs.append(Paragraph('Я оставляю за собой право отозвать свое согласие посредством составления соответствующего письменного документа, который может быть направлен мной в '
                          'адрес Оператора по почте заказным письмом с уведомлением о вручении либо вручен лично под расписку представителю Оператора. В случае получения моего '
                          'письменного заявления об отзыве настоящего согласия на обработку персональных данных, Оператор обязан прекратить их обработку в течение периода времени, '
