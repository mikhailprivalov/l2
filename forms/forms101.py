...
def form_18(request_data):
    ...
    objs.append(
        Paragraph('- Я осознаю что отказ от назначенного обследования и лечения могут осложнить процесс лечения и отрицательно сказаться на состоянии здоровья', style))
    objs.append(
        Paragraph(
    ...
    objs.append(
        Paragraph('- Разрешаю, в случае необходимости, предоставить информацию о диагнозе, степени тяжести и характере заболевания моим родственникам, законным представителям и следующим '
                  'гражданам:', style)
    )
    objs.append(Spacer(1, 2 * space))
    objs.append(HRFlowable(width=190 * mm, color=colors.black))
