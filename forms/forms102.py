...
#
#
#     num_contract = Napravleniya.objects.values_list('num_contract', flat=True).get(pk=ind_dir[0])
#     person_contract_obj = PersonContract.objects.get(num_contract=num_contract)
#     objs: List[Union[Spacer, Paragraph, Table, KeepTogether]] = []
#     barcode128 = code128.Code128(num_contract, barHeight=6 * mm, barWidth=1.25)
#
#     objs.append(Spacer(1, 11 * mm))
#
#     objs.append(Paragraph('ДОГОВОР &nbsp;&nbsp; № <u>{}</u>'.format(num_contract), styleCenter))
#     objs.append(Spacer(1, 1 * mm))
#     objs.append(Paragraph('НА ОКАЗАНИЕ ПЛАТНЫХ МЕДИЦИНСКИХ УСЛУГ НАСЕЛЕНИЮ', styleCenter))
#     styleTCenter = deepcopy(styleCenter)
#     styleTCenter.alignment = TA_CENTER
...
#     # представитель==Заказчик ()
#     payer_fio = None
#     p_payer = person_contract_obj.payer_card
#     p_agent = person_contract_obj.agent_card
#     patient_data = person_contract_obj.patient_data
#     if (p_payer is None) or (p_payer == p_agent ) or (p_agent and p_payer is None):
#         payer_fio = patient_data
#         is_pagent = True
#         p_agent_who = client_who + " (представитель пациента)"
#     else:
#         p_agent_who = "Представитель пациента"
...
#             client_side = ', с другой стороны, заключили в интересах Пациента (Потребителя)'
#         is_payer = True
#         payer_fio = p_agent.individual.fio()
#         objs.append(
#             Paragraph(
#                 '<font fontname ="PTAstraSerifBold">{}: </font> {}, дата рождения {} г., '
#                 'паспорт: {}-{} '
