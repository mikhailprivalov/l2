...
def get_confirm_direction(d_s, d_e, limit):
    with connection.cursor() as cursor:
        ...
        WHERE directions_issledovaniya.time_confirmation IS NOT NULL
          ...
          AND directions_istochnikifinansirovaniya.rmis_auto_send
          ...
          AND NOT (directions_napravleniya.rmis_number = 'NONERMIS'
                   OR directions_napravleniya.rmis_number = ''
                   OR directions_napravleniya.rmis_number IS NULL)
          AND directions_napravleniya.result_rmis_send = FALSE
          AND NOT (directions_napravleniya.imported_from_rmis = TRUE
                   AND directions_napravleniya.imported_directions_rmis_send = FALSE)
