...
def analyze_django_performance() -> Dict:
    ...
    if os.path.exists(settings_file):
        ...
        if "CACHES" in content and "memcached" in content:
            ...
            performance_settings["caching"]["recommendations"].append("Configure Memcached for better caching performance")
        ...
        if "CONN_MAX_AGE" in content:
            ...
            performance_settings["database"]["recommendations"].append("Add database connection pooling with CONN_MAX_AGE")
        ...
        if "ManifestStaticFilesStorage" in content:
            ...
            performance_settings["static_files"]["recommendations"].append("Use ManifestStaticFilesStorage for better static file handling")
        ...
        if "cached_db" in content:
            ...
            performance_settings["sessions"]["recommendations"].append("Use cached session backend for better performance")
        ...
        if "GZipMiddleware" in content:
            ...
            performance_settings["middleware"]["recommendations"].append("Add GZipMiddleware for response compression")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--json":
        ...
        data = {"bundles": analyze_webpack_bundles("assets/webpack_bundles"), "django": analyze_django_performance(), "gunicorn": check_gunicorn_config(), "timestamp": time.time()}
