...
def statistics_research(research_id, d_s, d_e, hospital_id_filter, is_purpose=0, purposes=None):
    ...
    t_iss AS
        ...
        users_doctorprofile.additional_info as additional_info,
    ...
    t_card AS
       (SELECT DISTINCT ON (clients_card.id) clients_card.id, clients_card.number AS num_card, 
        ...
        to_char(EXTRACT(YEAR from age(time_confirmation, date_born)), '999') as ind_age, t_hosp.title, t_iss.purpose_title, t_iss.vich_code, dir_category, iss_category, additional_info FROM t_iss
        LEFT JOIN t_card ON t_iss.client_id = t_card.id
