...
def directory_researches_list(request):
    ...
    for r in researches:
        autoadd = {lab["pk"]: [x["b__pk"] for x in direct.AutoAdd.objects.filter(a__pk=r["pk"], b__podrazdeleniye__pk=l["pk"]).values("b__pk")] for lab in labs}
        addto = {lab["pk"]: [x["a__pk"] for x in direct.AutoAdd.objects.filter(b__pk=r["pk"], a__podrazdeleniye__pk=l["pk"]).values("a__pk")] for lab in labs}
