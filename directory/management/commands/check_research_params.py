    ...
    def handle(self, *args, **kwargs):
                ...
                for r in researches:
                    ...
                    if fractions and len(fractions) > 0:
                        for f in fractions:
                            ...
                            print(f"{k}@{r.podrazdeleniye.title}@ {r.title}@ {research_code}@ {r.hide}@ - @{f.title}@ {fsli}@ {unit_title}@ {unit_code}")
        if type_researches == "is_paraclinic":
            ...
            for r in researches:
                ...
                print(f"{k}@{r.title}@ {r.short_title}@{r.podrazdeleniye.title}@{research_code}@ {nsi_id}")
