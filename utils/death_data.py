...
def get_death_data(research_id, date_start, date_end, exclude_hospital_id):
    with connection.cursor() as cursor:
        ...
        """,
            params={"research_id": research_id, "date_start": date_start, "date_end": date_end, "hospital_id": exclude_hospital_id, 'tz': TIME_ZONE},
        )
...
def find_death_by_dignoses(start_date, end_date, diagnoses, exclude_hospital_pk):
    result = get_death_data(DEATH_RESEARCH_PK, start_date, end_date, exclude_hospital_pk)
    directions = set()
    for i in result:
