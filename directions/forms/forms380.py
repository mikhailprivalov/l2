...
def form_01(c: Canvas, d: Napravleniya):
    def printForm(offset):
        ...
        c.drawCentredString(w / 2, py(58 + offset), "НА ВИЧ № {}{}".format(d.pk, d.external_number_suffix()))
...
def form_02(c: Canvas, dir: Napravleniya):
    def printForm():
        ...
        for i in range(0, 9):
            ...
            y += 5
        c.drawString(x_coord * mm, 250 * mm, "№ " + str(dir.pk) + dir.external_number_suffix())  # Номер направления
...
def form_03(c: Canvas, dir: Napravleniya):
    ...
    def printForm():
        ...
        c.drawString(100 * mm, 287 * mm, "№ - {}{}".format(dir.pk, dir.external_number_suffix()))
...
def form_05(c: Canvas, dir_obj: Union[QuerySet, List[Napravleniya]]):
    ...
    def printForm(dir):
        ...
        objs.append(tbl)
        objs.append(Spacer(1, 3 * mm))
        objs.append(Paragraph(f'НАПРАВЛЕНИЕ № {dir.pk}{dir.external_number_suffix()}', styleCenterBold))
...
def form_08(c: Canvas, dir_obj: Union[QuerySet, List[Napravleniya]]):
    ...
    def printForm(dir: Napravleniya):
        ...
        objs.append(Paragraph(f"Направление № {dir.pk}{dir.external_number_suffix()}", style=styleCenterBold))
