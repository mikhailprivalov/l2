...
class ProcedureList(models.Model):
    history_num = models.ForeignKey(Napravleniya, related_name='history_num', help_text='Номер истории', db_index=True, on_delete=models.CASCADE)
    diaries_num = models.ForeignKey(Napravleniya, related_name='diaries_num', help_text='Номер дневника', db_index=True, on_delete=models.CASCADE)
    ...
    times_medication = JSONField(default=None, blank=True, null=True, help_text="Время приема-статус-исполнитель")
...
class ProcedureListTimes(models.Model):
    procedure_list = models.ForeignKey(ProcedureList, null=True, blank=True, help_text='Назначение из процедурного листа', on_delete=models.SET_NULL)
    times_medication = models.DateTimeField(help_text='Время приема', db_index=True)
    executor = models.ForeignKey(DoctorProfile, default=None, blank=True, null=True, related_name="doc_who_create", help_text='Исполнитель', on_delete=models.SET_NULL)
