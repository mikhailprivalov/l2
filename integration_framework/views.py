...
def direction_data(request):
    ...
    for i in iss:
        ...
        if type_research == "paraclinic":
            if not i.research.podrazdeleniye:
                return Response({"ok": False})
            if not i.research.podrazdeleniye.p_type != 3:
                return Response({"ok": False})
...
def create_direction_by_param(body, request):
    ...
    diag_mcb10_data = directions.Diagnoses.objects.filter(d_type="mkb10.4", code=diag_mkb10).order_by("code").first()
...
def get_pdf_result_from_qr_code(request):
    ...
    if qrcode_prints == 3:
        Log(key=pk, type=5005, body=json.dumps({"data": data, "answer": 'Истекло количество попыток печати по данному QR-коду'}), user=None, application=app).save()
...
def get_price_data(request):
    ...
    if price:
        ...
        result = [
            {
                "title": i.research.title,
                "shortTitle": i.research.short_title,
                "coast": i.coast,
                "coastCito": i.coast_cito,
