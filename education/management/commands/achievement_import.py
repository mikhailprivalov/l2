...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        starts = False
        mmis_code, title, short_title, grade, year = '', '', '', '', ''
        for row in ws.rows:
            cells = [str(x.value) for x in row]
            if not starts:
                if "Код" in cells:
                    mmis_code = cells.index("КодДостижения")
                ...
                r = AchievementType.objects.filter(mmis_id=cells[mmis_code])
                if not r.exists():
                    AchievementType(
                        mmis_id=cells[mmis_code],
                        title=cells[title],
