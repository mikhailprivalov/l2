...
class TabelDocument(models.Model):
    ...
    comment_checking = models.TextField(blank=True, null=True, help_text="Комментарий от проверяющего")
    status = models.CharField(max_length=20, default=Status.DRAFT, db_index=True, choices=Status.choices, help_text="Статус")
    doc_change_status = models.ForeignKey(DoctorProfile, null=True, related_name="tabel_doc_change_status", blank=True, db_index=True, help_text="Профиль проверяющего", on_delete=models.SET_NULL)
    doc_change_status_string = models.CharField(max_length=64, null=True, blank=True, default=None)
    time_change_status = models.DateTimeField(null=True, blank=True, db_index=True, help_text="Время изменения статуса")
    ...
    def create_tabel_document(data, docprofile):
        ...
        department_pk = data.get("departmentPk")
        parent_document_pk = data.get("parentDocumentPk")
        with_confirm = data.get("withConfirm")
        version = 1
        parent_document = None
        if parent_document_pk:
            parent_document = TabelDocument.objects.get(pk=parent_document_pk)
            version = parent_document.version + 1

        tabel_document = TabelDocument(
            ...
            is_actual=True,
            parent_document_id=parent_document_pk,
            version=version,
        )
        if with_confirm:
            tabel_document.time_confirmation = timezone.now()
            tabel_document.status = TabelDocument.Status.CHECK
            if parent_document:
    ...
    def change_status_tabel_document(tabel_pk, data, docprofile):
        comment_checking = data.get("commentChecking", "")
        status = data.get("status", TabelDocument.Status.CHECK)
        tabel_document = TabelDocument.objects.filter(pk=tabel_pk).first()
        if tabel_document:
            tabel_document.doc_change_status = docprofile
            tabel_document.doc_change_status_string = docprofile.get_full_fio()
            tabel_document.time_change_status = timezone.now()
            tabel_document.comment_checking = comment_checking
            tabel_document.status = status
            tabel_document.save()


class TabelFactTimeWork(models.Model):
    class Status(models.TextChoices):
        WORK = "WORK", "Работа"
    ...
    status = models.CharField(max_length=20, default=Status.WORK, db_index=True, choices=Status.choices, help_text="Статус")

    class Meta:
        verbose_name = "Работник - время в табеле"
        verbose_name_plural = "Работники - время в табеле"


class TabelData(models.Model):
    """
    ...
    tabel_document = models.ForeignKey(TabelDocument, null=True, on_delete=models.SET_NULL)
    data_document = models.TextField(blank=True, null=True, help_text="Данные документа (JSON)")

    class Meta:
        verbose_name = "Табель с данными"
        verbose_name_plural = "Табели с данными"

    @staticmethod
    def save_fact_time_work_document(tabel_pk, data_document):
        document_fact_time_work = TabelData.objects.filter(pk=tabel_pk).first()
        if document_fact_time_work:
            document_fact_time_work.data_document = data_document
            document_fact_time_work.save()
    ...
    def get_fact_time_work_document(tabel_pk):
        ...
        result = {}
        if document_fact_time_work:
            document_data = json.loads(document_fact_time_work.data_document)
            person_data = document_data.get("personData") or []
            for person in person_data:
                employee_data = person.get("employeeData") or []
                for employee in employee_data:
                    tmp_dates = sorted(employee.get("dates", []).copy())
                    result[person][employee]["tmp_night_hours_dates"] = ["" for i in range(len(tmp_dates))]
                    result[person][employee]["tmp_night_hours"] = employee.get("nightHours", {})
                    result[person][employee]["tmp_common_hours"] = employee.get("commonHours", {})
