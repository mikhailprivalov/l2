...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadFractionFast: async function () {
      ...
      loadFieldsAndGroupsFast: async function () {
        if (!this.fieldsAndGroups) {
          return
        }
        const ids = this.fieldsAndGroups.ids.filter(i => !this.fieldsAndGroupsCache.hasOwnProperty(i))
        if (ids.length > 0) {
          this.loading = true
      ...
      updated() {
          if (this.mode === 'fraction' && !this.fractions.hasOwnProperty(this.value.trim())) {
            this.loadFraction();
