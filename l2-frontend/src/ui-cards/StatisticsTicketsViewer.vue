<template>
  <div style="height: 100%; width: 100%; position: relative">
    <div class="top-picker">
      <DateFieldNav
        style="width: 166px; align-self: stretch"
        :val.sync="date"
        :def="date"
      />
      <button
      ...
      <button
        class="btn btn-blue-nb"
        style="width: 94px; display: inline-block; align-self: stretch"
        @click="print"
      >
        <i class="glyphicon glyphicon-th-list" /> Экспорт
      </button>
    </div>
        ...
        >
          ...
          <tbody>
            ...
            >
              ...
              <td v-html="/*eslint-disable-line vue/no-v-html*/ row.info" />

              <td v-if="row.first_time">
                да
              </td>
              <td v-else>
                нет
              ...
              <td>{{ row.disp }}</td>
              <td class="doc">
                {{ row.doc }}<br>{{ row.department }}
              </td>

              <td class="control-buttons">
                <div
                  v-if="row.can_invalidate"
                  class="flex-wrap"
                >
                  <button
                    v-if="row.invalid"
            ...
            <tr v-if="data.length === 0">
              <td
                ...
                colspan="11"
              >
                За выбранную дату нет статталонов
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      statisticsTicketsPoint
        .loadTickets(this, 'date')
        .then((data) => {
          this.data = data.data;
        })
        .finally(() => {
