...
def form_02(direction: Napravleniya, iss: Issledovaniya, fwb, doc, leftnone, user=None):
    ...
    fwb.append(Spacer(1, 3 * mm))
    fwb.append(Paragraph('Наименование работодателя, адрес электронной почты, контактный номер телефона: '
                         f'{data["Наименование работодателя, почтовый адрес, адрес электронной почты, контактный номер телефона"]}', style))
    fwb.append(Spacer(1, 3 * mm))
    ...
    fwb.append(Spacer(1, 3 * mm))
    fwb.append(Paragraph(f'Вид (виды) деятельности, осуществляемый работником (в соответствии с приложением № 2 к Приказу М3 РФ от 20.05.2022г. № 342н) '
                         f'{data["Вид (виды) деятельности, осуществляемый работником ( в соответствии с приложением № 2 к Приказу М3 РФ от 20.05.2022г. № 324н)"]}', style))
    ...
    fwb.append(Paragraph(f'Результат психиатрического освидетельствования: '
                         f'{data["Результат психиатрического освидетельствования: Пригоден к следующим видам деятельности согласно приказа М3 РФ от 20.05.2022г. № 324Н"]}', style))
