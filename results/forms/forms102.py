...
def form_02(direction, iss, fwb, doc, leftnone, user=None):
    ...
    fwb.append(Paragraph(f'Заключение № {direction.pk} ', styleCenterBold))
    fwb.append(Paragraph('предварительного (периодического) медицинского осмотра', styleCenterBold))
    short_title = iss.research.short_title
    fwb.append(Paragraph(f'{short_title.upper()}', styleCenterBold))
